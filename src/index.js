...
<td align="right" style="font-size:22px;font-weight:bold;">₹${total}</td>
</tr>
...
<h3 style="color:#2B1B14;">Items</h3>
...
<tr>
<td style="background:#2B1B14;padding:30px;text-align:center;color:#ffffff;">

<p style="margin:0;">Thank you for choosing Idexcy.</p>

<p style="margin-top:10px;color:#E8DDD3;">Crafted with purpose.</p>

<a href="https://idexcy.com" style="display:inline-block;margin-top:20px;padding:14px 30px;background:#ffffff;color:#2B1B14;text-decoration:none;border-radius:8px;font-weight:bold;">Visit Idexcy</a>

<p style="margin-top:30px;color:#A89C92;font-size:13px;">© ${new Date().getFullYear()} Idexcy<br>Questions? Just reply to this email.</p>
...
`,
      ...
      `)
				...
				return Response.json(
					{ success: true, message: 'Product added successfully', productId: result.meta.last_row_id },
					{ headers: corsHeaders },
				);
			} catch (err) {
				return Response.json(
		...
		if (request.method === 'GET' && url.pathname === '/api/check-resend') {
			return Response.json({ keyExists: !!env.RESEND_API_KEY, keyPrefix: env.RESEND_API_KEY?.substring(0, 8) });
		}
		...
		if (request.method === 'GET' && url.pathname === '/api/orders') {
			...
			const orders = await env.d1_server
				.prepare(`
            SELECT id, amount, payment_status, order_status, created_at
            FROM orders
            WHERE user_id = ?
        ...
        `)
			...
			return Response.json({ success: true, orders: orders.results }, { headers: corsHeaders });
		}
		...
		if (request.method === 'PUT' && url.pathname.startsWith('/api/orders/')) {
			try {
				...
				const result = await env.d1_server
					.prepare(`
        UPDATE orders
        SET customer_name = ?, email = ?, phone = ?, address = ?, city = ?, state = ?, pincode = ?
        WHERE id = ?
      `)
				...
				return Response.json({ success: true, message: 'Order updated successfully' }, { headers: corsHeaders });
			} catch (err) {
		...
		if (request.method === 'PUT' && url.pathname.startsWith('/api/users/')) {
			try {
				...
				if (!name || !email) return Response.json({ success: false, message: 'Name and email are required' }, { status: 400, headers: corsHeaders });
				if (is_admin !== 0 && is_admin !== 1) return Response.json({ success: false, message: 'is_admin must be 0 or 1' }, { status: 400, headers: corsHeaders });

				const existingUser = await env.d1_server.prepare(`SELECT id FROM users WHERE id = ?`).bind(userId).first();
				if (!existingUser) return Response.json({ success: false, message: 'User not found' }, { status: 404, headers: corsHeaders });

				const emailUser = await env.d1_server.prepare(`SELECT id FROM users WHERE email = ? AND id != ?`).bind(email, userId).first();
				if (emailUser) return Response.json({ success: false, message: 'Email is already being used by another user' }, { status: 409, headers: corsHeaders });
				...
				return Response.json({ success: true, message: 'User updated successfully', user: { id: userId, name, email, is_admin } }, { headers: corsHeaders });
			} catch (err) {
		...
		if (request.method === 'GET' && url.pathname === '/api/admin/products') {
			try {
				...
				return Response.json({ success: true, products: products.results }, { headers: corsHeaders });
			} catch (err) {
				return Response.json({ success: false, error: err.message }, { status: 500, headers: corsHeaders });
			}
		}
		...
		if (request.method === 'GET' && url.pathname === '/api/admin/orders') {
			try {
				const orders = await env.d1_server.prepare(`
        SELECT o.id, o.amount, o.payment_status, o.order_status, o.created_at, u.name, u.email
      ...
      `).all();
		...
		if (request.method === 'PUT' && url.pathname.startsWith('/api/admin/products/')) {
			try {
				...
				if (result.meta.changes === 0) return Response.json({ success: false, message: 'Product not found' }, { status: 404, headers: corsHeaders });
				return Response.json({ success: true, message: 'Product updated successfully' }, { headers: corsHeaders });
			} catch (err) {
				return Response.json({ success: false, error: err.message }, { status: 500, headers: corsHeaders });
			}
		}
		...
		if (request.method === 'GET' && url.pathname === '/api/user/orders') {
			try {
				const userId = url.searchParams.get('userId');
				if (!userId) return Response.json({ success: false, message: 'User ID is required' }, { status: 400, headers: corsHeaders });
				const orders = await env.d1_server.prepare(`SELECT * FROM orders WHERE user_id = ? ORDER BY created_at DESC`).bind(userId).all();
				for (const order of orders.results) {
					const items = await env.d1_server.prepare(`
          SELECT oi.id, oi.product_id, p.name, p.image, oi.quantity, oi.price
          FROM order_items oi JOIN products p ON oi.product_id = p.id WHERE oi.order_id = ?
        `).bind(order.id).all();
					order.items = items.results;
				}
				return Response.json({ success: true, totalOrders: orders.results.length, orders: orders.results }, { headers: corsHeaders });
			} catch (err) {
				return Response.json({ success: false, error: err.message }, { status: 500, headers: corsHeaders });
			}
		}
		...
		if (request.method === 'PUT' && url.pathname.startsWith('/api/admin/orders/') && url.pathname.endsWith('/status')) {
			try {
				...
				if (!validStatuses.includes(order_status)) return Response.json({ success: false, message: 'Invalid order status' }, { status: 400, headers: corsHeaders });
				...
				if (!order) return Response.json({ success: false, message: 'Order not found' }, { status: 404, headers: corsHeaders });

				const result = await env.d1_server.prepare(`UPDATE orders SET order_status = ? WHERE id = ?`).bind(order_status, orderId).run();
				if (result.meta.changes === 0) return Response.json({ success: false, message: 'Order status was not updated' }, { status: 400, headers: corsHeaders });

				let emailSubject = `Your Idexcy Order #${orderId} Update`;
				let emailHeading = `Your order is now ${order_status}`;
				let emailMessage = '';
				if (order_status === 'Received') emailMessage = "We've received your order and will begin preparing it shortly.";
				if (order_status === 'In Transit') emailMessage = 'Good news! Your Idexcy order is on its way.';
				if (order_status === 'Delivered') emailMessage = 'Your Idexcy order has been delivered. We hope you enjoy every bite.';
				if (order_status === 'Unable to Reach') emailMessage = 'We were unable to reach you while attempting to deliver your Idexcy order. Our delivery team may try again soon.';
				...
				try {
					const resendResponse = await fetch('https://api.resend.com/emails', {
						...
						body: JSON.stringify({
							...
							html: `
              <!DOCTYPE html><html><body style="margin:0;padding:0;background:#f7f3ef;font-family:Arial,sans-serif;color:#2b1b14;">
                <div style="max-width:600px;margin:40px auto;background:#ffffff;padding:40px;">
                  <h1 style="margin:0 0 25px;font-family:Georgia,serif;font-size:28px;">IDEXCY</h1>
                  <p style="font-size:16px;">Hi ${order.customer_name || 'there'},</p>
                  <h2 style="font-family:Georgia,serif;font-size:24px;">${emailHeading}</h2>
                  <p style="font-size:16px;line-height:1.6;">${emailMessage}</p>
                  <div style="margin:30px 0;padding:20px;background:#f7f3ef;">
                    <p style="margin:0 0 8px;"><strong>Order #:</strong> ${order.id}</p>
                    <p style="margin:0 0 8px;"><strong>Status:</strong> ${order_status}</p>
                    <p style="margin:0;"><strong>Order Total:</strong> ₹${order.amount}</p>
                  </div>
                  <p style="font-size:14px;line-height:1.6;color:#666;">Thank you for choosing Idexcy.</p>
                  <p style="font-size:14px;color:#666;">Crafted with purpose.</p>
                </div>
              </body></html>
            `,
						}),
					...
					else console.error('Failed to send email:', resendData);
				} catch (emailError) { console.error('Email sending error:', emailError); }

				return Response.json({ success: true, message: 'Order status updated successfully', orderId, order_status, emailSent }, { headers: corsHeaders });
			} catch (err) {
		...
		if (request.method === 'GET' && url.pathname.startsWith('/api/orders/')) {
			try {
				...
				if (!order) return Response.json({ success: false, message: 'Order not found' }, { status: 404, headers: corsHeaders });
				const items = await env.d1_server.prepare(`
        SELECT oi.id, oi.product_id, p.name, p.image, oi.quantity, oi.price, (oi.quantity * oi.price) AS subtotal
        FROM order_items oi JOIN products p ON oi.product_id = p.id WHERE oi.order_id = ?
      `).bind(orderId).all();
				order.items = items.results;
				return Response.json({ success: true, order }, { headers: corsHeaders });
			} catch (err) {
				return Response.json({ success: false, error: err.message }, { status: 500, headers: corsHeaders });
			}
		}
